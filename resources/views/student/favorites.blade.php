<x-layouts.app :title="__('Mis Favoritos')">
    <div class="flex flex-col gap-6">
        <div class="flex items-center justify-between">
            ...
            <div class="text-sm text-gray-500 dark:text-gray-400">
                {{ $favorites->total() }} {{ __('favoritos en total') }}
            </div>
        </div>
                            ...
                            <div class="flex-1">
                                ...
                                @if($favorite->favoritable)
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                                        {{ $favorite->favoritable->title ?? $favorite->favoritable->name ?? 'Sin título' }}
                                    </h3>
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                                        <span>{{ __('Agregado a favoritos:') }} {{ $favorite->created_at ? $favorite->created_at->diffForHumans() : 'Fecha no disponible' }}</span>
                                        @if(isset($favorite->favoritable->created_at) && $favorite->favoritable->created_at)
                                            <span>{{ __('Creado:') }} {{ $favorite->favoritable->created_at->format('d/m/Y') }}</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="text-gray-500 dark:text-gray-400 italic">
                                        {{ __('Contenido no disponible') }}
                                    </div>
                                @endif
                            </div>
        ...
        @else
            <div class="text-center py-12">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
                    {{ __('No tienes favoritos aún') }}
                </h3>
                <p class="text-gray-500 dark:text-gray-400 mb-6">
                    {{ __('Cuando agregues contenido a favoritos, aparecerá aquí.') }}
                </p>
